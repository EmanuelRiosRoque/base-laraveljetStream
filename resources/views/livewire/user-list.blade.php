<div class="mt-4">
    <section class="container px-4 mx-auto">
        <div class="sm:flex sm:items-center sm:justify-between">
            <div>
                ...
                <div class="flex items-center gap-x-3">
                    <h2 class="text-lg font-medium text-gray-800 ">Roles Usuarios</h2>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 ">
                                @foreach ($users as $user)
                                    ...
                                    <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                        <div>
                                            <h2 class="font-medium text-left text-gray-800">{{$user->id}}</h2>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                        <div>
                                            <h2 class="font-medium text-left text-gray-800">{{$user->role->rol}}</h2>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 text-sm whitespace-nowrap">
                                        <x-button class="bg-green-500 hover:bg-green-700" wire:click="openModalEditar( {{$user->id}} )" wire:loading.attr="disabled">
                                            Editar
                                        </x-button>
        ...
        <x-dialog-modal class="flex items-center justify-center" wire:model.live="modalEditar">
            <x-slot name="title">
                {{ isset($this->user->id) ? 'Edit Item'  : 'Create Item' }}
            </x-slot>

            <x-slot name="content" class="mt-3">
                <div class="col-span-6 mb-3 sm:col-span-4">
                    <x-label for="name" value="{{ __('Name') }}" />
                    <x-input id="name" type="text" class="block w-full mt-1" wire:model="name" autocomplete="name" disabled/>
                    <x-input-error for="name" class="mt-2" />
                </div>

                <div class="col-span-6 sm:col-span-4 mb-4">
                    <x-label class="" for="name" value="{{ __('Rol ?') }}" />
                    <select wire:model="rol" name="rol" id="rol" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <x-slot name="footer">
                <x-secondary-button wire:click="$set('modalEditar',false)" wire:loading.attr="disabled">
                    {{ __('Cancelar') }}
                </x-secondary-button>
